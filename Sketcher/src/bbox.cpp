...
using namespace std;
BBox :: BBox(vector<Triangle> triangles)
:mMin(0,0,0),
mMax(0,0,0),
mCenter(0,0,0)
{
...
void BBox::calculateBBox(vector<Triangle> triangles)
{
   ...
   cout<<mMin.x()<<" "<<mMin.y()<<" "<<mMin.z()<<endl;
   cout<<mMax.x()<<" "<<mMax.y()<<" "<<mMax.z()<<endl;
   cout<<mCenter.x()<<" "<<mCenter.y()<<" "<<mCenter.z()<<endl;
}
